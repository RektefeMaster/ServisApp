-- `resolve_session` e-posta parametresini taşımayı bırakıyor.
--
-- 0015'ten beri kimlik bağlama YALNIZ telefonla yapılıyor: doğrulanmış bir
-- e-posta tek başına personel/yönetici üyeliğini ele geçirmeye yetmemeli
-- (SPEC §2). O günden beri `p_email` gövdede hiç okunmuyordu; imzada durması
-- "e-postanın bir yerde işe yaradığı" izlenimi veriyordu.
--
-- Yayın sırasında eski sürüm hâlâ 3 argümanlı çağrı yapabilir; bu yüzden eski
-- imza silinmez, yeni gövdeye devreden ince bir kabuk olarak bırakılır.
...
-- Eski imza: yalnız geriye dönük uyumluluk için, e-postayı yok sayar.
create or replace function resolve_session(
  p_auth_user_id uuid,
  p_phone text,
  p_email text
...
set search_path = public, pg_temp
as $$
  select resolve_session(p_auth_user_id, p_phone);
$$;

revoke all on function resolve_session(uuid, text) from public;
revoke all on function resolve_session(uuid, text, text) from public;
alter function resolve_session(uuid, text) owner to servisapp_definer;
alter function resolve_session(uuid, text, text) owner to servisapp_definer;
grant execute on function resolve_session(uuid, text) to servisapp_api;
