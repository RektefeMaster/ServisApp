-- pg-boss şeması ve aylık partition bakımı.
-- Worker CREATE TABLE public'de yapamaz; partition DDL definer fonksiyondadır.

create schema if not exists pgboss;

grant usage, create on schema pgboss to servisapp_worker;
grant usage on schema pgboss to servisapp_api;
...
begin
  if exists (select 1 from pg_roles where rolname = 'anon') then
    execute 'revoke all on schema pgboss from anon';
  end if;
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    execute 'revoke all on schema pgboss from authenticated';
  end if;
  if exists (select 1 from pg_roles where rolname = 'service_role') then
    execute 'revoke all on schema pgboss from service_role';
  end if;
end
$$;
